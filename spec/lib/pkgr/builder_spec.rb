...
describe Pkgr::Builder do
  ...
  describe "#package" do
    ...
    it "builds the proper fpm command" do
      ...
      expect(command).to include("fpm -s dir --verbose --force --exclude '**/.git**' -C \"#{builder.build_dir}\" -n \"my-app\" --version \"0.0.1\" --iteration \"#{config.iteration}\" --url \"#{config.homepage}\" --provides \"my-app\"")
